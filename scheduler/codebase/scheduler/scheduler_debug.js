...
webix.protoUI({
	...
	_setUnits: function(){
		...
		while(start.valueOf()<end.valueOf()){
			...
			var result = this._correctEventsDates(start,scheduler.getEvents(start, next));
			if(result.length)
	...
	webix.protoUI({
		...
		getTodayMultiEvents:function(){
			...
			scheduler.syncData(this, d, next,function(ev){
				return webix.Date.datePart(ev.start_date,true).valueOf() != webix.Date.datePart(ev.end_date,true).valueOf();
			});
			this.resize();
		},
		...
		_initMonth:function(){
			...
			this.$$("calendar").attachEvent("onAfterZoom",function(zoom){
				if(!zoom)
					this.callEvent("onAfterMonthChange",[this._settings.date]);
			});
			this.$$("calendar").attachEvent("onAfterMonthChange",webix.bind(function(date){
				if(!this.$$("calendar")._zoom_level){
					var today = new Date();
